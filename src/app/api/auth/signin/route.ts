/**
 * POST /api/auth/signin — start a session (Level 13).
 *
 * On success the current anonymous session's conversations are claimed for the
 * signing-in user. Both halves of that decision come from the server: the user
 * id from a token Supabase verified, the session id from this server's own
 * httpOnly cookie. Nothing the client sent takes part.
 */

import { AuthError, buildAuthCookies, claimAnonymousConversations, signIn } from '@/lib/auth';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json(
      { authenticated: true, email: session.email, claimedConversations: claimed },
      { status: 200, headers },
    );
  } catch (caught) {
    ...
    log.error('auth.signin_failed', { requestId, error: caught });
