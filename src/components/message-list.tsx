...
import { describeSupportedFormats } from '@/lib/documents-client';
...
function WelcomeHero({ onSuggestion }: WelcomeHeroProps) {
  return (
    <section className="relative mx-auto flex w-full max-w-2xl flex-col items-center px-2 py-10 text-center sm:py-16">
      ...
      {/*
        The formats belong on the empty state because this is the one screen a
        new user reads. Previously they existed only in the attach button's
        tooltip, which is invisible on a touch device and to anyone navigating
        by keyboard — so the answer to "what can I upload?" was unreachable for
        a large share of users.
      */}
      <p className="animate-rise-delay-2 mt-4 max-w-md text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">
        Attach a document with the paperclip and ask questions about it.
        <br />
        <span className="text-zinc-400 dark:text-zinc-500">{describeSupportedFormats()}</span>
      </p>

      <p className="animate-rise-delay-2 mt-4 max-w-md text-xs leading-relaxed text-zinc-400 dark:text-zinc-500">
        Answers are grounded in the documents you upload, with citations back to the passage
        they came from.
      </p>
    </section>
  );
